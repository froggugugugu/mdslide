...
- **ワークスペースはフォルダ**。`deck.md` `images/` `master.pptx` を 1 フォルダに置く。外部のプロセスが `deck.md` を書き換えると自動で再読み込みする
...
1. 資料用のフォルダを作り、レイアウト名を規約どおりに付けた `master.pptx` を置く(見本: `examples/sample-master.pptx`)
2. 「フォルダを開く」。`deck.md` が無ければ雛形ができる
