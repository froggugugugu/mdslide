...
2 回目からは前回の資料がそのまま開く。作業中に別の資料を開くときは、メニューの「ファイル」→「資料を開く…」(⌘O)かツールバーのファイル名のボタンから、資料のフォルダか Markdown ファイルを選ぶ。新しい資料は「ファイル」→「新しい資料を作る…」(⌘N)で作る。前回のファイルが無ければ起動画面になり、「最近開いたもの」から選べる。使い方は ⌘/ で開く(初回の起動では自動で開く)。
...
開いた Markdown の親フォルダが作業の単位になる。「資料を開く」で Markdown ファイルを選んだときはその名前のまま使い、フォルダを選んだときと「新しい資料を作る」では `deck.md` になる(無ければ表紙・章・スライド 1 枚だけの空の枠で作る)。
