...
- ツールバーはアイコンボタンに揃える。主操作の書き出しだけは青い `.btn.icon.primary`。使い方は作業中のツールバーに置かず、メニューの「ヘルプ」・⌘/・起動画面から開く（初回は自動で開く）。新しい資料・資料を開くは、起動画面に加えてメニューの「ファイル」（⌘N / ⌘O。Mac では ⌘ だけにして Ctrl は Vim に残す）から作業中もできる。
...
tools/          export_pptx.py (deck.json + master.pptx → out.pptx, python-pptx。画像は --assets の中だけ)  mdslide_draw.py (theme.json 準拠の図生成。ワークスペースに配布)  convert_master.py (手持ちのテンプレート pptx → マスター。元ファイルを土台に意匠はそのまま、役割レイアウト名と、本文ページの版面・文字だけ見本に揃える。--profile で見本を選ぶ。ADR-0030 / ADR-0033)  master_profiles.py (見本の版面と字の段。make_sample_master.py と convert_master.py が共有する。配布アプリに同梱されるのは tools/ だけなのでここに置く。ADR-0032 / ADR-0033)
...
scripts/        install.sh (配布版を入れる。Releases の zip を https で取り、GitHub が記録した sha256 と署名を確かめて /Applications へ、版を固定した python-pptx を wheel だけで venv へ。Pages の直下に置く。ADR-0022)  make_demo_gif.py (実アプリを CDP で操作して README のデモ GIF を再生成)  make_tour_images.py (同じく実アプリを操作し、紹介ページのカルーセル画像 docs/media/tour/*.webp を撮る)  make_icon.py (docs/media/icon.svg から build/icon.png・icon.icns を生成)  make_sample_master.py (発表用 sample-master.pptx と報告用 report-master.pptx を、一つのグリッドの 2 プロファイルで作る。ADR-0023 / ADR-0031 / ADR-0032)  make_decorated_master.py (見本に Pillow の生成画像などの装飾を足して examples/decorated-master.pptx を作る)
...
examples/       sample-master.pptx（発表用の見本。レイアウト名規約、16:9、本文 18pt）  report-master.pptx（報告用の見本。本文 11pt、色は 3 色、タイトルは結論 1 文が 2 行入る帯。ADR-0032）※2 つとも scripts/make_sample_master.py で生成  decorated-master.pptx（ロゴ・帯・画像の表紙・フォント変更を足した見本。プレビューと書き出しの確認用。scripts/make_decorated_master.py で生成）
```
