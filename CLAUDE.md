...
tools/          export_pptx.py (deck.json + master.pptx → out.pptx, python-pptx。画像は --assets の中だけ)  mdslide_draw.py (theme.json 準拠の図生成。ワークスペースに配布)  convert_master.py (手持ちのテンプレート pptx → マスター。元ファイルを土台にテーマ・装飾はそのまま、役割レイアウト名と本文枠だけ揃える。ADR-0030)
