    ...
    <div className="scrim" onClick={onClose}>
      <div className="sheet help" onClick={(e) => e.stopPropagation()}>
        ...
        <ol>
          ...
          <li>{isElectron
            ? <>起動画面の「新しい資料を作る」で資料のフォルダを選ぶ（その場で作れる）と、中に <code>deck.md</code> を空の枠（表紙・章・スライド 1 枚）で作る。既存の資料は「資料を開く」でフォルダか Markdown ファイルを選ぶ（<code>deck.md</code> の無いフォルダも空の枠から始まる）</>
            : <>起動画面の「フォルダを開く」で資料のフォルダを選ぶ。中の <code>deck.md</code> を使い、無ければ空の枠（表紙・章・スライド 1 枚）から作る</>}</li>
          <li>右のエディタで書く（既定は Vim キーバインド。設定の「エディタ」で通常のテキスト編集に切り替え可）。左のサムネイルはドラッグか <kbd>{alt}</kbd> <kbd>↑</kbd> <kbd>↓</kbd> で並べ替える。番号は自動</li>
          <li>ツールバー右端の青い書き出しボタンで <code>out/deck.pptx</code>。Python と python-pptx が必要で、見つからなければ起動時に案内が出る（入れ方は設定の「書き出し」）</li>
        </ol>
