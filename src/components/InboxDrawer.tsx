...
import { attachInboxFs, useInboxStore } from "../console/inboxStore";
  ...
  return (
    <aside className={`inbox ${dragOver ? "drag" : ""}`} data-testid="inbox" onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={onDrop}>
      <div className="inbox-bar">
        ...
        <button className="btn quiet" onClick={toggle} aria-label="下書きを閉じる">閉じる</button>
      </div>
      ...
      <div className="inbox-row">
        <button className="btn quiet" onClick={() => { if (fs) void flush(fs).then(newDraft); }} disabled={!fs || !draft.trim()}>新しいメモ</button>
        ...
        <button className="btn quiet" onClick={() => fileRef.current?.click()} disabled={!fs}>ファイルを追加</button>
      </div>
      <div className="inbox-notes" aria-label="notes 一覧">
        ...
        {notes.filter((n) => !n.endsWith(".keep")).map((n) => (
          <div key={n} className={`note ${selected.has(n) ? "on" : ""}`}>
            <label><input type="checkbox" checked={selected.has(n)} onChange={() => pick(n)} aria-label={`${n} を渡す`} /> <code>{n.replace("notes/", "")}</code></label>
            <button className="link" onClick={() => fs && void remove(fs, n)} aria-label={`${n} を削除`}>削除</button>
          </div>
        ))}
        <div className="console-hint">チェックしたものだけ渡します（未選択なら全部）。</div>
      </div>
