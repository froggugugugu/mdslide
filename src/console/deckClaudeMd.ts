/** Conventions dropped into the workspace as CLAUDE.md so an interactive Claude Code session edits the deck file correctly. */
export function deckClaudeMd(deckFile = "deck.md"): string {
  return `# このフォルダについて（mdslide）

このフォルダはスライド資料です。唯一の正は \`${deckFile}\`、画像は \`images/\`、書式は \`master.pptx\`。
\`${deckFile}\` を編集すると mdslide が即座に再表示します。別の Markdown ファイルは作らないでください。
...
- ${deckFile} を書き換える前の版は mdslide が \`.mdslide/history/\` に取っている。気にせず上書きしてよい
- 変更内容は短く報告する
`;
}

/** The conventions for the default file name. */
export const DECK_CLAUDE_MD = deckClaudeMd();

export async function ensureDeckClaudeMd(backend: { exists(rel: string): Promise<boolean>; writeText(rel: string, text: string): Promise<number> }, deckFile = "deck.md"): Promise<boolean> {
  if (await backend.exists("CLAUDE.md")) return false;
  await backend.writeText("CLAUDE.md", deckClaudeMd(deckFile));
  return true;
}
