...
export const PROMPTS: PromptTemplate[] = [
  { id: "format", label: "整形して deck.md に", hint: "notes/ の口語メモをスライドの Markdown に",
    text: "notes/ にあるメモ（参照を付けたファイル）を読んで、CLAUDE.md の規約に従い deck.md をスライド資料として整形してください。章立て（#）と本文スライド（##）に分け、1スライドは短い箇条書き 3〜6 行、口語は書き言葉に直し、内容は足さず削らず。既存の deck.md があれば構成を尊重して差し替えてください。図が必要な箇所は ![TODO 図の説明]() と書いてください。" },
  ...
  { id: "summarize", label: "要約", hint: "notes/ を 5 行に要約してエグゼクティブサマリのスライドに",
    text: "notes/ のメモ（参照を付けたファイル）を読み、要点を 5 行以内に要約して、deck.md の先頭の章の直後に「## サマリー」スライドとして追加してください。CLAUDE.md の規約に従い、数字と固有名詞は落とさないでください。" },
...
export function buildPrompt(id: string, notes: string[], slideRef?: string | null): string {
  const p = PROMPTS.find((x) => x.id === id);
  if (!p) throw new Error(`unknown prompt ${id}`);
  const refs = notes.map((n) => `@${n}`).join(" ");
  const parts = [p.text.replace(/\s+/g, " ").trim()];
